...
**🎯 精准作业台 V2 优化：**
- ✅ 统一锚点格式：`<!-- @block:block_xxx x,y,width,height -->`
- ✅ 移除 react-pdf 依赖，统一使用 PNG 图片预览（减少 ~2MB 依赖）
- ✅ 简化 Bounding Box 坐标计算，直接使用 `bbox * scale`
- ✅ 基于锚点的同步滚动，编辑器光标位置与预览面板精确同步
- ✅ 复杂表格（colspan/rowspan）保留 HTML 格式，简单表格输出 Markdown
- ✅ 8 个属性测试（Property-Based Testing）全部通过
- ✅ 263 前端测试 + 13 后端测试全部通过

**📋 单据类型配置功能（2026-01-27）：**
- ✅ 新增单据类型管理：发票、合同、收据、身份证、出差报告等预设类型
- ✅ 步骤2上传区域添加单据类型下拉选择器
- ✅ 单据设定弹窗：支持新增/编辑/删除单据类型，配置关键词字段和检查点问题
- ✅ 步骤5自动加载：根据步骤2选择的单据类型自动执行数据提取和检查点验证
- ✅ 历史缓存恢复：加载历史任务时自动恢复单据类型选择

**🔧 状态管理修复：**
- ✅ 修复步骤5/6之间数据传递问题（统一使用 `window.stateManager`）
- ✅ 修复历史缓存加载后单据类型未正确恢复的问题
- ✅ 删除重复的 `loadCachedJob` 方法，统一使用 `globalFunctions.js` 中的实现

> 详细更新记录请查看 [CHANGELOG.md](CHANGELOG.md)

## 历史更新 (2026-01-26)

**🎨 前端 V3 架构升级（91 项任务全部完成）：**
- ✅ CSS 模块化：样式拆分为 `base.css`, `layout.css`, `steps.css`, `panels.css`, `components.css`
- ✅ 核心服务：新增 `EventBus.js`（事件总线）、`StateManager.js`（状态管理）、`StepManager.js`（步骤管理）
- ✅ 步骤组件化：`Step1ModelLoad.js` ~ `Step6Confirmation.js` 六步工作流组件
- ✅ 面板组件：`HistoryPanel.js`, `TemplatePanel.js`, `CheckpointPanel.js`
- ✅ UI/UX 增强：步骤进度条动画、面板展开动画、加载指示器、Toast 通知
- ✅ 响应式布局：支持桌面、平板、手机多端适配
- ✅ V3 API 扩展：修正记录、检查点、模板管理等新接口
- ✅ 完整测试覆盖：前端 193 测试 + 后端 37 测试全部通过
...
│ ┌──────────┐  ┌─────────────────────────────────────────────┐  ┌────────────┐ │
│ │ 状态指示  │  │  ① ─── ② ─── ③ ─── ④ ─── ⑤ ─── ⑥           │  │ 📋 单据设定 │ │
│ │ ● OCR模型 │  │ 模型  上传  识别  预录入 数据  财务          │  │ 📂 历史缓存 │ │
│ │ ● LLM服务 │  │ 加载  文件  处理        提取  确认          │  └────────────┘ │
│ │ ● RAG服务 │  └─────────────────────────────────────────────┘                 │
│ └──────────┘                                                                   │
├────────────────────────────────────────────────────────────────────────────────┤
│                    拖放或点击上传 PDF/JPG/PNG 文件                               │
│                    [选择单据类型 ▼] 发票/合同/收据/身份证/出差报告               │
├─────────────────────────────────┬──────────────────────────────────────────────┤
│                                 │  📋 识别结果  [📦Block] [📝Markdown]          │
│    📷 Original Document         │  ┌─────────────────────────────────────────┐ │
...
步骤5 数据提取界面：
┌────────────────────────────────────────────────────────────────────────────────┐
│  📊 数据提取: ✅已完成 | ✅ 检查点验证: 已完成    [📋LLM日志] [🚀提交到财务确认] │
├─────────────────────────────────┬──────────────────────────────────────────────┤
│  📊 数据提取                     │  ✅ 检查点验证                               │
│  选择提取模板：                  │  输入验证问题（每行一个）：                   │
│  [发票] [合同] [收据] [出差报告] │  ┌─────────────────────────────────────────┐ │
...
│  ┌──────────────────────────────────────────────────────────────────────────┐ │
│  │ ① 📄 R202412-0024.pdf                                                    │ │
│  │    📋 出差报告  🕐 2026-01-27 18:14  ⏱ 27s                    [98%] [🗑] │ │
│  └──────────────────────────────────────────────────────────────────────────┘ │
│  ┌──────────────────────────────────────────────────────────────────────────┐ │
│  │ ② 📄 6200029517.pdf                                                      │ │
│  │    📋 发票  🕐 2026-01-27 17:30  ⏱ 33s                        [92%] [🗑] │ │
│  └──────────────────────────────────────────────────────────────────────────┘ │
└────────────────────────────────────────────────────────────────────────────────┘
```
...
│   ├── models/             # 数据模型
│   ├── tests/              # 单元测试
│   │   ├── test_v3_api.py          # V3 API 测试（24 用例）
│   │   ├── test_v3_integration.py  # V3 集成测试（13 用例）
│   │   └── ...                     # 其他测试
│   ├── app.py              # 应用入口
│   ├── config.py           # 配置文件
│   └── requirements.txt    # Python 依赖
├── frontend/               # JavaScript 前端（纯原生 JS + CSS）
│   ├── src/
│   │   ├── styles/         # CSS 模块化样式
│   │   │   ├── base.css        # CSS 变量和重置样式
│   │   │   ├── layout.css      # 主布局样式
│   │   │   ├── steps.css       # 工作流步骤进度条样式
│   │   │   ├── panels.css      # 面板样式
│   │   │   └── components.css  # 组件样式
│   │   ├── services/       # 前端核心服务
...
│   │   ├── components/     # 组件
│   │   │   ├── steps/          # 步骤组件
│   │   │   │   ├── Step1ModelLoad.js   # 模型加载
│   │   │   │   ├── Step2FileUpload.js  # 文件上传
│   │   │   │   ├── Step3Recognition.js # 智能识别
│   │   │   │   ├── Step4PreEntry.js    # 预录入
│   │   │   │   ├── Step5DataExtract.js # 数据提取
│   │   │   │   └── Step6Confirmation.js # 财务确认
│   │   │   ├── panels/         # 面板组件
...
│   │   ├── __tests__/      # 前端单元测试
│   │   │   ├── EventBus.test.js    # 事件总线测试（15 用例）
│   │   │   ├── StateManager.test.js # 状态管理测试（20 用例）
│   │   │   └── StepManager.test.js  # 步骤管理测试（25 用例）
│   │   ├── index.html      # 主页面
│   │   └── index.js        # 应用入口
...
PP-ChatOCRv4 采用 RAG（检索增强生成）架构，分为两个阶段：

**阶段一：文档索引（OCR 完成后自动执行）**
```
文档文本 → 分块（500字符/块） → 向量化（BGE模型） → 存入向量数据库（ChromaDB）
```

**阶段二：智能问答/提取（用户触发）**
```
用户输入关键词/问题 → 向量化 → 从向量数据库检索相关片段（Top-K）
                                        ↓
              检索到的相关片段 + 用户问题 → 发给 LLM → 得到答案
```

这种架构的优势：
- **精准检索**：只发送相关内容给 LLM，避免上下文过长
- **支持长文档**：即使文档有几十页，也能快速定位相关信息
- **节省成本**：减少 LLM 处理的 token 数量
